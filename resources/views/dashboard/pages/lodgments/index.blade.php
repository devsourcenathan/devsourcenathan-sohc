                ...
                <tbody>
                    @forelse ($lodgments as $lodgment)
                        <tr>
                            ...
                            <td>
                                @if ($lodgment->state == 0)
                                    <a href="lodgments/details/publish/{{ $lodgment->id }}" type="button"
                                        class="badge rounded-pill bg-success cursor-pointer">Publier</a>
                                @else
                                    <a href="lodgments/details/unpublish/{{ $lodgment->slug }}" type="button"
                                        class="badge rounded-pill bg-warning cursor-pointer">Masquer</a>
                                @endif

                                <a href="lodgments/details/{{ $lodgment->slug }}"
                                    class="badge rounded-pill bg-info cursor-pointer" style="cursor: pointer;">Afficher</a>
                                <a href="lodgments/update/{{ $lodgment->slug }}"
